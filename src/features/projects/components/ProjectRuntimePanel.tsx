...
import { ChevronDown, ChevronUp, Eye, Loader2, Play, RotateCw, Square, Trash2, Plus, TerminalSquare } from "lucide-react";
...
export function ProjectRuntimePanel() {
  ...
  const run = store.runs[store.activeCwd] ?? null;
  ...
  useEffect(() => {
    ...
    void subscribePreviewReady((target) => {
      ...
      const current = s.runs[s.activeCwd];
  ...
  const handleStart = async () => {
    const previewResult = await store.start();
    if (previewResult) setShowConfirm(true);
  };
