...
-- 使用 IF NOT EXISTS 确保幂等（表可能已通过其他途径创建）
...
CREATE INDEX IF NOT EXISTS idx_recent_items_type
...
-- Git 文件状态表（缓存）
CREATE TABLE IF NOT EXISTS git_file_status (
    id TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_git_file_status_repo
...
CREATE INDEX IF NOT EXISTS idx_command_history_executed
ON command_history(last_executed_at DESC);

CREATE INDEX IF NOT EXISTS idx_command_history_count
