...
/// 获取页面详情（资源 + 页面扩展 + 全部块）。
#[tauri::command]
pub fn get_page(state: State<AppState>, resource_id: String) -> CommandResult<serde_json::Value> {
    let conn = lock_db(&state);
    let resource = repo::get_resource(&conn, &resource_id)?
        .ok_or_else(|| AppError::new("not_found", format!("页面 {resource_id} 不存在")))?;
    let page = page_service::get_page(&conn, &resource_id)?
        .ok_or_else(|| AppError::new("not_page", format!("{resource_id} 不是页面")))?;
...
pub fn rename_page(
    ...
    let _ = app.emit(
        EVENT_RESOURCE_CHANGED,
        serde_json::json!({ "parent_id": updated.parent_id }),
    );
    Ok(updated)
}
...
/// 供测试使用的辅助函数（验证页面创建逻辑）。
#[cfg(test)]
pub fn _page_lifecycle_for_test(
    conn: &rusqlite::Connection,
) -> rusqlite::Result<crate::db::models::Page> {
